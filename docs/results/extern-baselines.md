...
`scripts/run_extern_baselines.py` full (15 seeds).
...
the same planner. Part C is the computation-only row.
...
its own map; evaluated as-is against the true drifting optimum:
...
(10x10 unknown terrain, rho=0.02, budget 20, 15 seeds, all goal=100%)
