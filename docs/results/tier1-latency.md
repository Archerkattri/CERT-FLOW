...
*D\* Lite incremental repair scales with the perturbed region, not the graph — speedup over from-scratch Dijkstra grows with |V| at fixed locality, and degrades honestly when changes are global.*

**Reproduce:** `scripts/run_tier1_latency.py`

> **Finding —** At fixed locality, incremental repair gets *relatively* faster as the graph grows (best speedup on the largest grid in the moving regime). The win is conditional: when the changed region approaches graph scale (largest radius on the smallest grid) it inverts to a slowdown. Across every round of every cell, incremental cost equals scratch cost (zero mismatches).

Full run (200 rounds/cell). Raw:
...
Rows ordered best -> worst by speedup.
...
**Reproduce:** `scripts/run_repeated_queries.py`
...
Rows ordered best -> worst by engine/networkx ratio (lower = our engine wins by more).
